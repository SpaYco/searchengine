...
describe 'Searches', type: :feature do
  ...
  it 'is valid with valid data' do
    search = Search.new(query: 'test', session_id: SecureRandom.urlsafe_base64(16))
    expect(search).to be_valid
  end

  it 'is not valid without query' do
    search = Search.new(session_id: SecureRandom.urlsafe_base64(16))
    expect(search).to_not be_valid
  end
  
  it 'is not valid if query is less than 3' do
    search = Search.new(query: 'te', session_id: SecureRandom.urlsafe_base64(16))
    expect(search).to_not be_valid
  end
  
  it 'is not valid if query is more than 50' do
    search = Search.new(query: long_paragraph, session_id: SecureRandom.urlsafe_base64(16))
    expect(search).to_not be_valid
  end
